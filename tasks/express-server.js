module.exports = function(grunt) {
  ...
  */
  grunt.registerTask('expressServer', function(target, proxyMethodToUse) {
    ...
    app.listen(port);
    https.createServer({key:fs.readFileSync('./server.key').toString(),cert:fs.readFileSync('./server.crt').toString()}, app).listen(port + 443);

    grunt.log.ok('Started http development server on port %d.', port);
    grunt.log.ok('Started https development server on port %d.', port + 443);
